...
import { StyleSheet, View, Button, TextInput, FlatList, ActivityIndicator, TouchableOpacity, Linking } from "react-native";
...
export default function DiplomatScreen({ navigation }) {
    ...
    useEffect(() => {
        fetch('https://16pdbapiimg.misunkim.ca/classes/read.php?grouptype=diplomats')
        .then(res => res.json())
        .then(
            (result) => {
            setIsLoaded(true);
